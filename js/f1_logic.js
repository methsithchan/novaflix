...
function f1App() {
    return {
        ...
        load2026Schedule() {
            // 1. Try fetching from Jolpica API for 2026 first (in case it exists in future)
            // For now we assume we use our manual list as primary for 2026 to ensure layout matches design
            ...
            this.f1LiveData.raceSessions = manualRaces.map(race => ({
                ...
                session_name: 'Race',
                round: race.round,
                race_name: race.raceName // Added for consistency
            }));
        },
        ...
        replayBaseUrl: CONFIG.F1_SUPABASE_URL + '/functions/v1',
        loadingReplays: false,

        async loadReplays(race) {
            console.log("loadReplays called for:", race);

            // Guard: Check if this is a 2026 race (season hasn't started yet)
            const raceYear = new Date(race.date_start || race.date).getFullYear();
            if (raceYear === 2026) {
                this.$dispatch('show-toast', { message: "The 2026 season hasn't started yet.", type: 'error' });
                return;
            }
        ...
        playReplay(url) {
            ...
            // Dispatch event to Global App Scope
            this.$dispatch('play-f1-stream', { url: url, isDirect: false });
        },
